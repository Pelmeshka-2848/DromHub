...
namespace DromHub.Models
{
    ...
    public class LocalStock
    {
        ...
        /// <summary>
        /// Служебная заметка о позиции склада, позволяющая операторам фиксировать источник данных или временные допущения.
        /// Используйте поле для аналитических пометок, которые помогают объяснить, почему позиция имеет текущую цену или остаток.
        /// Обновляйте текст при каждом ручном вмешательстве, чтобы избежать устаревших гипотез и упростить аудит.
        /// </summary>
        /// <value>Строка длиной до 4000 символов; по умолчанию — пустая строка без пробелов.</value>
        /// <remarks>
        /// Nullability: значение никогда не бывает <see langword="null"/> — при инициализации устанавливается <see cref="string.Empty"/>.
        /// Потокобезопасность: изменять только в рамках одного контекста EF.
        /// Побочные эффекты: обновление свойства помечает сущность как изменённую в <see cref="DromHub.Data.ApplicationDbContext"/>.
        /// </remarks>
        [Column("note")]
